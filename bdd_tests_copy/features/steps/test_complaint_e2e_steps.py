...
@scenario(
    '../features/complaint_workflow_end_to_end_test.feature',
...
def send_message_if_text_available(test_context: Dict[str, Any], column_name: str):
    ...
    # Clear previous request/response to ensure a clean state for each step
...
def verify_response_if_executed(test_context: Dict[str, Any], expected_key: str):
    ...
    preceding step was actually executed (i.e., a request was made).
    """
    if test_context.get('request') is None:
        logger.info(f"SKIPPED verification for '{expected_key}' because the preceding step was skipped or failed.")
        return
...
def send_initial_request(test_context: Dict[str, Any]):
    ...
    test_context['last_response'] = response # Set this for the first verification
...
# The rest of the 'when' steps call the same helper for consistency
